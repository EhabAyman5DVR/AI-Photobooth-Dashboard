...
import { AuthState, User, UserRole, Project } from './types';
import { initializeStore, getStoreData, setStoreData } from './store';
import { 
  LayoutDashboard, 
  Users, 
  FolderKanban, 
...
const App: React.FC = () => {
  const [auth, setAuth] = useState<AuthState>({
    user: JSON.parse(localStorage.getItem('pb_auth') || 'null'),
    isAuthenticated: !!localStorage.getItem('pb_auth')
  });
    ...
    <HashRouter>
      <Layout user={auth.user!} onLogout={logout}>
        <Routes>
          ...
          <Route path="/projects" element={<ProjectList user={auth.user!} />} />
          <Route path="/projects/:id" element={<ProjectDetail user={auth.user!} />} />
        ...
        <header className="h-16 bg-white/80 backdrop-blur-sm border-b border-slate-200 px-6 flex items-center justify-between sticky top-0 z-30">
          ...
          <div className="flex items-center gap-2">
            <h1 className="text-lg font-semibold text-slate-800">
              {navItems.find(n => n.path === location.pathname)?.label || 'Project Details'}
            </h1>
          </div>
