...
import { Project, User, UsageLog, UserRole, CloudinaryImage } from '../types';
...
const ProjectDetail: React.FC<ProjectDetailProps> = ({ user }) => {
  ...
  useEffect(() => {
    ...
    if (found.cloudinaryCloudName && found.cloudinaryTag) {
      fetchImages(found.cloudinaryCloudName, found.cloudinaryTag);
    }
  }, [id, user, navigate]);

  const fetchImages = async (cloudName: string, tag: string) => {
    setLoadingImages(true);
    try {
      // Cloudinary Resource List API (Client Side)
      // Note: Requires "Resource List" feature to be enabled for the specific tag
      const response = await fetch(`https://res.cloudinary.com/${cloudName}/image/list/${tag}.json`);
      if (response.ok) {
        const data = await response.json();
        setImages(data.resources || []);
      } else {
        console.warn('Cloudinary list fetching failed. It may be due to security settings.');
        // Fallback to empty if not enabled
        setImages([]);
      }
    } catch (err) {
  ...
  const handleSimulateApiCall = () => {
    ...
    setTimeout(() => {
      ...
      // Refresh images if project is configured
      if (project.cloudinaryCloudName && project.cloudinaryTag) {
        fetchImages(project.cloudinaryCloudName, project.cloudinaryTag);
      }
    }, 800);
  };
            ...
            <div className="bg-white p-8 rounded-2xl border border-slate-200 shadow-sm relative overflow-hidden">
              ...
              <div className="flex items-center justify-between mb-8">
                ...
                <div className={`px-4 py-1.5 rounded-full text-xs font-bold uppercase tracking-widest ${
                  project.status === 'active' ? 'bg-emerald-50 text-emerald-600' : 'bg-red-50 text-red-600'
                }`}>
                  {project.status}
                </div>
              </div>
              ...
              <div className="flex items-center justify-between mb-6">
                ...
                <button 
                  ...
                  className={`flex items-center gap-2 px-6 py-2 rounded-xl font-bold transition-all ${
                    isSimulating || project.status === 'exhausted'
                ...
                <div>
                  ...
                  <div className="flex items-center gap-2 bg-slate-800 p-3 rounded-xl border border-slate-700">
                    <code className="text-indigo-400 text-sm flex-1 truncate">
                      https://api.booth.ai/v1/projects/{project.id}/generate
                    </code>
                    <button 
                      onClick={() => copyToClipboard(`https://api.booth.ai/v1/projects/${project.id}/generate`)}
